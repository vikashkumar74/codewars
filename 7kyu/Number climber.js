...
// with N and such that every number in the sequence is either the double of the preceeding number or the double plus 1.
...
function climb(n){
    // ...
    let arr=[];
      arr.push(Math.trunc(n))
    while(n>1){
      arr.push(Math.trunc(n/2));
      n=Math.trunc(n/2);
    }
